...
from PyQt5.Qt import (QApplication, QWidget, QPushButton, QThread,QMutex, pyqtSignal, QTimer)
import time
...
class mgf_events_class(object):
    def __init__(self, main_ui = mgf_ui_main.Ui_MainWindow()):
        ...
        self.progressBar_links()
        print("Events handlers initialized done.")

    def get_version(self):
        forge_ver1 = self.__main_ui.lineEdit_ver1.text()
        forge_ver2 = self.__main_ui.lineEdit_ver2.text()
        forge_ver3 = self.__main_ui.lineEdit_ver3.text()

        if (forge_ver1 and forge_ver2 and forge_ver3):
            forge_ver = forge_ver1 + "." + forge_ver2 + "." + forge_ver3
        else :
            forge_ver = default_forge_ver
        self.__main_ui.label_ver.setText("当前选择版本: " + forge_ver)

    # btn_sure_events
    def btn_sure_links(self):
        print("btn sure events links.")
        self.__main_ui.btn_sure.clicked.connect(self.btn_sure_clicked)
    ...
    def lineEdit_ver_links(self):
        ...
        self.__main_ui.lineEdit_ver1.textChanged.connect(self.lineEdit_ver_changes)
        self.__main_ui.lineEdit_ver2.textChanged.connect(self.lineEdit_ver_changes)
        self.__main_ui.lineEdit_ver3.textChanged.connect(self.lineEdit_ver_changes)
    ...
    def timer_operate(self):
        ...
        if (self.cnt%5 == 0):
            ...
            if (self.info_flag == 0):
                self.__main_ui.label_github.setText(github_hint)
        if (self.info_flag == 1):
            self.__main_ui.label_github.setText('已运行时间: ' + str(self.cnt) + '秒')
